...
from app.live import is_direct_upstream_request, is_live_request
...
class FootballDataService:
    ...
    async def get(self, endpoint: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if is_live_request(endpoint, params):
            return await self.get_live(endpoint, params)
    ...
    async def sync(self, endpoint: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if not is_live_request(endpoint, params):
            cached = self.cache.get(endpoint, params)
            if cached is not None:
